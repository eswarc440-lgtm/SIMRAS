...
def train_models(
    ...
    *,
    ...
    health_persistence_predictions = np.clip(
        test["condition_rating"].to_numpy(dtype=float),
        0,
        9,
    )
    ...
    metrics = {
        ...
        "health_persistence_mae_rating": float(
            mean_absolute_error(
                test["next_condition_rating"],
                health_persistence_predictions,
            )
        ),
    ...
    manifest = {
        ...
        "evaluation_protocol": {
            "split_unit": "bridge_key",
            "partition_method": "stable_hash_bridge_partition",
            "train_rule": "partition < 70",
            "calibration_rule": "70 <= partition < 85",
            "test_rule": "partition >= 85",
            "bridge_identity_leakage_control": True,
            "test_partition_held_out": True,
            "risk_calibration_uses_test": False,
            "health_persistence_baseline": "current condition rating predicts next condition rating",
        },
        "evaluation_counts": {
            "all_train_rows": int(len(train)),
            "all_calibration_rows": int(len(calibration)),
            "all_test_rows": int(len(test)),
            "all_train_bridges": int(train["bridge_key"].nunique()),
            "all_calibration_bridges": int(calibration["bridge_key"].nunique()),
            "all_test_bridges": int(test["bridge_key"].nunique()),
            "risk_train_rows": int(len(risk_train)),
            "risk_calibration_rows": int(len(risk_calibration)),
            "risk_test_rows": int(len(risk_test)),
            "risk_train_bridges": int(risk_train["bridge_key"].nunique()),
            "risk_calibration_bridges": int(risk_calibration["bridge_key"].nunique()),
            "risk_test_bridges": int(risk_test["bridge_key"].nunique()),
            "rul_train_rows": int(len(rul_train)),
            "rul_test_rows": int(len(rul_test)),
            "rul_train_bridges": int(rul_train["bridge_key"].nunique()),
            "rul_test_bridges": int(rul_test["bridge_key"].nunique()),
        },
        ...
        "limitations": [
            ...
            "RUL is conditional on observed deterioration events and is subject to censoring bias",
            "RUL is not a structural failure-time forecast and must not independently schedule rehabilitation",
