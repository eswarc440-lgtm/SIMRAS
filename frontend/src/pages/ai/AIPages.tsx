  ...
  return (
    <Shell title="AI Intelligence" description="Stored model outputs and governance-aware decision support. Unavailable values remain N/A or withheld.">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="High Risk Assets" value={overview ? overview.high_risk_assets.toLocaleString() : "â€”"} icon={ShieldAlert} tone="danger" delta="Risk score â‰¥ 70" />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <ChartCard title="Highest Stored Risk" subtitle="Current database ranking">
          <ul className="divide-y">
            {insights.length === 0 ? <li className="py-4 text-sm text-muted-foreground">No stored risk predictions are available.</li> : insights.slice(0, 6).map((asset) => <li key={asset.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 py-3 first:pt-0"><div><p className="text-sm font-medium">{asset.name ?? asset.id}</p><p className="mt-1 text-xs text-muted-foreground">{asset.id} Â· {asset.district ?? "District N/A"}</p><p className="mt-1 font-mono text-[11px] text-muted-foreground">Confidence {asset.confidence_score != null ? `${Math.round(asset.confidence_score * 100)}%` : "N/A"}</p></div><div className="text-right"><RiskBadge risk={riskFrom(asset.risk_score)} /><p className="mt-2 text-sm font-semibold">{fmt(asset.risk_score)}</p></div></li>)}
          </ul>
        </ChartCard>
      </div>
    </Shell>
...
export function PredictionsPage() {
  ...
  useEffect(() => {
    apiRequest<{ items?: PredictionItem[]; predictions?: PredictionItem[] }>("/api/v1/predictions?limit=100")
      .then((payload: any) => setItems(payload.items ?? payload.predictions ?? []))
      .catch(() => setItems([]));
  }, []);
...
export function ModelPerformancePage() {
  ...
  return (
    <Shell title="Model Performance & Governance" description="Values are read from the live model registry/status endpoint; no hardcoded RÂ², MAE or RMSE values are shown.">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
