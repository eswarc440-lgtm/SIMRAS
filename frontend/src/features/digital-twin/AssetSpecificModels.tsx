  ...
  return (
    <group>
      ...
      <Badge position={[0, 4.7, 0]}>
        <strong>{twin.asset.name}</strong>
        <br />
        Berth/quay: {berthM} m Ã‚Â· permissible draft: {draftM} m
      </Badge>
    </group>
  );
}
...
export function AssetSpecificModel(props: Props) {
  ...
  /*
   * SIMRAS_VERIFIED_TOPOLOGY_OVERRIDE
   *
   * Sir Arthur Cotton Barrage uses verified four-arm topology
   * from verifiedWaterTopologyTwin instead of generic barrage
   */
  if (twin.asset.asset_code === "AP_BAR_WRIS_B00131") {
    return <GatedBarrage {...props} />;
  }
