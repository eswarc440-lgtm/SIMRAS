    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Total Assets" value={overview ? overview.total_assets.toLocaleString() : "—"} icon={Building2} delta="Current registry" />
          <StatCard label="Average Health" value={overview ? pct(overview.average_health_score) : "—"} icon={BarChart3} tone="success" delta="Stored health predictions only" />
          <StatCard label="High Risk" value={overview ? overview.high_risk_assets.toLocaleString() : "—"} icon={ShieldAlert} tone="danger" delta="Stored risk score ≥ 70" />
              ...
              {highRisk.length === 0 ? (
                ...
                return (
                  <li key={asset.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 py-3 first:pt-0">
                    <div className="min-w-0"><p className="truncate text-sm font-medium">{asset.name ?? asset.id}</p><p className="mt-1 text-xs text-muted-foreground">{asset.id} · {asset.district ?? "District N/A"}</p><p className="mt-1 font-mono text-[11px] text-muted-foreground">Confidence {asset.confidence_score != null ? `${Math.round(asset.confidence_score * 100)}%` : "N/A"}</p></div>
