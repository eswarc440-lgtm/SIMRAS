...
export function ReportsPage() {
  ...
  const selectedTemplate = templates.find((item) => item.id === selectedReportId);
                    ...
                    <Select value={selectedReportId} onValueChange={setSelectedReportId}>
                      ...
                      <SelectContent>
                        {templates.map((report) => (
                          <SelectItem key={report.id} value={report.id}>{report.title} · {report.format}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                      ...
                      <Select value={selectedAssetId} onValueChange={setSelectedAssetId}>
                        ...
                        <SelectContent>
                          {assets.map((asset) => {
                            ...
                            return <SelectItem key={code} value={code}>{code} · {asset.name ?? asset.asset_type ?? "Asset"}</SelectItem>;
                          })}
                        </SelectContent>
                      </Select>
                    </div>
              ...
              return (
                <article key={report.id} className="flex h-full flex-col rounded-lg border bg-card p-5 shadow-panel">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0"><h2 className="text-sm font-semibold">{report.title}</h2><p className="mt-1 text-[11px] text-muted-foreground">{report.category} · {report.format}</p></div>
                  </div>
...
export function ReportDetailsPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    apiRequest<{ items?: ReportTemplate[] }>("/api/v1/reports/catalog")
      .then((payload) => setReport((payload.items ?? []).find((item) => item.id === id) ?? null))
      .finally(() => setLoading(false));
  }, [id]);
          ...
          <>
            ...
            <div className="rounded-lg border bg-card p-6">
              <dl className="grid gap-4 sm:grid-cols-3">
                ...
                <div><dt className="text-xs text-muted-foreground">Data source</dt><dd className="mt-1 font-medium">FastAPI → PostgreSQL/PostGIS</dd></div>
              </dl>
