...
export function MaintenancePage() {
  ...
  useEffect(() => {
    const loadMaintenance = async () => {
      try {
        ...
        // Support both 'items' and 'maintenance' keys from backend
        const data = response.items || response.maintenance || [];
  ...
  const getStatusBadge = (status: string | undefined) => {
    ...
    switch (s) {
      ...
      default:
        return <span className="text-xs">{status || "Unknown"}</span>;
    }
  };
  ...
  const getPriorityBadge = (priority: string | undefined) => {
    ...
    switch (p) {
      ...
      default:
        return <span className="text-xs">{priority || "N/A"}</span>;
    }
  };

  const getCost = (record: MaintenanceRecord): number | undefined => {
    const cost = record.cost ?? record.estimated_cost ?? record.actual_cost;
    if (typeof cost === 'string') {
      return parseFloat(cost);
    }
    return cost as number | undefined;
  };
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {record.description && <p className="mt-2 line-clamp-2 text-sm">{record.description}</p>}
                      <div className="mt-2 flex flex-wrap gap-4 text-sm text-muted-foreground">
                        {getCost(record) && (
                          <div className="flex items-center gap-1">
                            <DollarSign className="size-3.5" />
                            ₹{getCost(record)?.toLocaleString()}
                          </div>
                        )}
                        {(record.assigned_contractor || record.performed_by) && (
                          <div>{record.assigned_contractor || record.performed_by}</div>
                        )}
                        {(record.planned_start_date || record.maintenance_date) && (
                          <div className="flex items-center gap-1">
                            <Clock className="size-3.5" />
                            {new Date(record.planned_start_date || record.maintenance_date || "").toLocaleDateString()}
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="rounded-lg border bg-card p-6">
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <dd className="mt-1">{selectedRecord.maintenance_type || "N/A"}</dd>
              </div>
              <div>
                <dt className="font-semibold text-muted-foreground">Status</dt>
                <dd className="mt-1">{selectedRecord.status ? getStatusBadge(selectedRecord.status) : "N/A"}</dd>
              </div>
              <div>
                <dt className="font-semibold text-muted-foreground">Priority</dt>
                <dd className="mt-1">{selectedRecord.priority ? getPriorityBadge(selectedRecord.priority) : "N/A"}</dd>
              </div>
              {getCost(selectedRecord) && (
                <div>
                  <dt className="font-semibold text-muted-foreground">Cost</dt>
                  <dd className="mt-1">₹{getCost(selectedRecord)?.toLocaleString()}</dd>
                </div>
              )}
              {(selectedRecord.assigned_contractor || selectedRecord.performed_by) && (
                <div>
                  <dt className="font-semibold text-muted-foreground">Assigned To</dt>
                  <dd className="mt-1">{selectedRecord.assigned_contractor || selectedRecord.performed_by}</dd>
                </div>
              )}
              {(selectedRecord.planned_start_date || selectedRecord.maintenance_date) && (
                <div>
                  <dt className="font-semibold text-muted-foreground">Date</dt>
                  <dd className="mt-1">{new Date(selectedRecord.planned_start_date || selectedRecord.maintenance_date || "").toLocaleString()}</dd>
                </div>
              )}
